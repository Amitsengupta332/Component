...
export default function HeroSection2o() {
  // ✅ Carousel slides (FIXED)
  const slides = [heroMock, heroMock2, heroMock3, heroMock4];
              ...
              <div
                ...
                {slides.map((img, idx) => (
                  <div key={idx} className="relative min-w-full h-full">
                    ...
                    {/* OPTIONAL: keep heroMock on top always
                       (Uncomment if you want background slide + same mock overlay) */}
                    {/* 
                    <img
                      src={heroMock}
                      alt="Product preview"
                      className="absolute inset-0 w-full h-full object-contain select-none drop-shadow-[0_18px_40px_rgba(0,0,0,0.20)]"
                      draggable={false}
                    />
                    */}
                  </div>
